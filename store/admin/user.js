...
export const actions = {
  ...
  // store users data
  storeUser({ commit, dispatch }, payload) {
    // set promise
    return new Promise((resolve, reject) => {
      // store data to Rest API "/api/admin/users" with method "POST"
      this.$axios
  ...
  getDetailUser({ commit }, payload) {
    // set promise
    return new Promise((resolve, reject) => {
      // get data from Rest API "/api/admin/users/:id" with method "POST"
      this.$axios
  ...
  updateUser({ dispatch, commit }, { userId, payload }) {
    // set promise
    return new Promise((resolve, reject) => {
      // store data to Rest API "/api/admin/users/:id" with method "POST"
      this.$axios
        .post(`/api/admin/users/${userId}`, payload)

        // success
        .then(() => {
          // dispacth action "getUsersData"
          dispatch("getUsersData");
